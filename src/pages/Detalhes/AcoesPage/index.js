...
import {agendarOs, finalizarService} from '../../../data/services/OrdemServicosServices';
import {colors} from '../../../styles';

import {Container, InputsContainer, SigantureImageContainer, SigantureModalContetn, SigantureModalOverlay, SignatureButton, SignatureButtonsContainer, SignatureButtonText, TextAreaContainer, TextAreaErrorMessage, TextAreaSubTitle, Title} from './styles';
import ControlledInput from '../../../components/ControlledInput';
import ControlledDateTimePicker from '../../../components/ControlledDateTimePicker';
import ControlledCheckBox from '../../../components/ControlledCheckBox';
import { FloatingAction } from 'react-native-floating-action';
import { Icon } from 'react-native-elements';
import { ActivityIndicator, Alert, Modal, View, Text, Image, useWindowDimensions } from 'react-native';
...
function AcoesPage({osId, CodAtendimento}) {
  ...
  async function finalizar(formData, acao = "fechar") {
    ...
    if(substituiuPeca === null) {
      Alert.alert('Por favor preencha o campo "Foi necessário substituir peça?" e tente novamente.');
      return;
    }
    
    if(substituiuPeca) {
      if(!formData.pecasubstituida?.trim() || !formData.pecadoequipamento?.trim()) {
        Alert.alert('Por favor preencha os campos sobre a substituição de peça e tente novamente.');
        return;
      }
    }
    
    if(!formData.data_visita) {
      Alert.alert('Por favor preencha o campo "Data da visita" e tente novamente.');
      return;
    }
    
    if(!formData.pessoaquerecebeutecnico?.trim()) {
      Alert.alert('Por favor preencha o campo "Recebido por" e tente novamente.');
      return;
    }

    if(formData.observacoes?.length > 150) {
      Alert.alert('Por favor respeite o limite de 150 caracteres no campo "Observações".');
      return;
    }
  ...
  return (
    ...
    <Container>
      ...
      <InputsContainer>
        ...
        <TextAreaContainer>
          <TextAreaSubTitle
            errorStyle={observacoesWatcher?.length > 150}
          >
          ...
          <ControlledInput
            ...
            name="observacoes"
            label="Observações"
            errorMessage={observacoesWatcher?.length > 150 && 'Limite de 150 caracteres'}
            textarea
          />
        </TextAreaContainer>

        {
          signaturePreview && (
            <SigantureImageContainer>
              <Image source={{uri: signaturePreview}} resizeMode="contain" style={{width: '100%', height: 200}} />
            </SigantureImageContainer>
          )
        }
