...
export default function Detalhes({navigation, route}) {
  ...
  const renderInfoPage = useCallback((props) => <InfoPage {...props} osData={generateOsData(osData)}  />, [osData]);
  const renderAcoesPage = useCallback((props) => <AcoesPage {...props} osId={osData.ORDEM} CodAtendimento={osData.CodAtendimento} />, [osData]);
  const renderMensuracaoPage = useCallback((props) => <Mensuracao {...props} osId={osData.ORDEM} osData={generateOsData(osData)} />, [osData]);

  return (
    <View style={{flex: 1}}>
      <TabNavigator.Navigator backBehavior="none" initialRouteName="Info" tabBarOptions={{indicatorStyle: {backgroundColor: colors.primary}, activeTintColor: colors.primary}}>
        <TabNavigator.Screen
